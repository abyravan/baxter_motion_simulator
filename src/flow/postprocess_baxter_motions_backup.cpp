...
int main(int argc, char **argv)
{
    ...
    */
    ...
    std::vector<std::vector<float> > valid_joint_positions  = read_csv_data(load_folder + "/positions.csv", valid_joint_names);
    ...
    std::vector<std::vector<float> > valid_joint_velocities = read_csv_data(load_folder + "/velocities.csv", valid_joint_names);
    ...
    std::vector<std::vector<float> > valid_joint_efforts = read_csv_data(load_folder + "/efforts.csv", valid_joint_names);
    ...
    std::vector<std::vector<float> > valid_commanded_joint_velocities = read_csv_data(load_folder + "/commandedvelocities.csv", valid_joint_names);
    ...
    std::vector<std::vector<float> > parameter_vector = read_csv_data(load_folder + "/parameters.csv", parameter_names);
    for(int i = 0; i < parameter_names.size(); i++)
        parameter_map[parameter_names[i]] = parameter_vector[0][i]; // Get the parameters

    // Get the timestamps from the files
    std::vector<float> timestamps = read_csv_data(load_folder + "/positions.csv", "time");
    ...
    cameradatafile << Eigen::Matrix4f(glM) << endl;
    ...
    int numdatasets = 0, numdiscarded = 0;
    while(d_end < valid_commanded_joint_velocities.size()-1) // till we reach the end of our velocity data
    {
        ...
        {
            float dist = vect_norm(sub(valid_commanded_joint_velocities[d_start], valid_commanded_joint_velocities[d_end]), 2); // Distance between controls
            if (dist > 0) break;
            d_end++;
        }
        ...
        std::vector<float> time_traj(timestamps.begin() + d_start, timestamps.begin() + d_end + 1);
        float start_time = time_traj[0];
        ...
        float traj_time = timestamps[d_end] - timestamps[d_start];
        ...
        std::vector<std::vector<float> > joint_angle_traj(valid_joint_positions.begin() + d_start, valid_joint_positions.begin() + d_end + 1);
        std::vector<std::vector<float> > joint_angle_traj_interp = interpolateTrajectory(joint_angle_traj, time_traj, numframes);
        assert(joint_angle_traj_interp.size() == numframes);

        /// == Interpolate the joint velocities to get data at 30Hz frame rate
        std::vector<std::vector<float> > joint_velocity_traj(valid_joint_velocities.begin() + d_start, valid_joint_velocities.begin() + d_end + 1);
        std::vector<std::vector<float> > joint_velocity_traj_interp = interpolateTrajectory(joint_velocity_traj, time_traj, numframes);
        assert(joint_velocity_traj_interp.size() == numframes);

        /// == Interpolate the joint efforts to get data at 30Hz frame rate
        std::vector<std::vector<float> > joint_effort_traj(valid_joint_efforts.begin() + d_start, valid_joint_efforts.begin() + d_end + 1);
        std::vector<std::vector<float> > joint_effort_traj_interp = interpolateTrajectory(joint_effort_traj, time_traj, numframes);
        assert(joint_effort_traj_interp.size() == numframes);

        /// == Get the commanded joint velocity (single control vector)
        std::vector<float> commanded_joint_velocities = valid_commanded_joint_velocities[d_start];
        ...
        printf("Dataset: %d (Discarded: %d) => Start: %d/%d, End: %d/%d, Time: %f, Num images: %d \n", numdatasets, numdiscarded, (int) d_start,
        ...
        postprocessstats << selfcollision << " " << d_start << " " << d_end << " "
                         << numframes << " " << traj_time << " " << motion_dir << endl;
