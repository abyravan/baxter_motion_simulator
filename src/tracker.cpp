...
void Tracker::setFrameOnGPU(const int frame) {
    ...
    if (frame > 0)
    {
        _pcSource->setFrame(frame-1); // Set previous frame
        _pcSource->advance(); // Advance so that GPU data is loaded
    }
    else
    {
        _pcSource->setFrame(0);
    }
}
...
/*
void Tracker::removePrior(int model,int frame)
{


    for(int i=0; i < _priors.size(); i++){
        Point3D3DPrior *prior = reinterpret_cast<Point3D3DPrior*>  ( _priors[i]);
        if ( (prior->getSourceFrame()==frame )  && (prior->getSourceModelID()==model))
            _priors.erase(_priors.begin() + i);
    }
